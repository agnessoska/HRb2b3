-- Update AI prompt to v3 for structured JSON generation
-- Focus on interactive workspace format without timer
...
ЗАДАЧА:
Создай структурированный план интервью в формате JSON. План должен быть практичным инструментом для интервьюера с возможностью делать заметки.
...
7. **Экранирование:** Если в тексте есть кавычки - используй одинарные (') вместо двойных (")
8. **Язык:** Весь контент должен быть на языке: {language}
9. **Без эмодзи**

ВАЛИДАЦИЯ:
- Проверь что JSON валидный
- Все поля заполнены
- time_allocation суммарно = estimated_duration
- Минимум 5 секций (intro, experience, psychometric, motivation, conclusion)',
  true
)
...
-- Update model config for structured interview (use better model for complex JSON)
