-- Обновляем промпт для анализа резюме для получения максимально детального и инсайтного анализа
UPDATE ai_prompts
...
ЗАДАЧИ АНАЛИЗА:
1.  **Executive Summary**: Дай краткую, емкую характеристику (2-3 предложения). Кто этот человек профессионально?
2.  **Match Score**: Объективно оцени соответствие требованиям (0-100%). Будь строг.
3.  **Hard Skills**: Выдели ключевые сильные стороны и КРИТИЧЕСКИЕ пробелы (чего не хватает для вакансии).
4.  **Soft Skills**: Оцени "мягкие навыки" на основе опыта и стиля описания (коммуникация, лидерство, инициативность).
5.  **Cultural Fit**: Насколько кандидат впишется в корпоративную культуру (судя по тону резюме, хобби, волонтерству).
6.  **Red Flags**: Найди скрытые риски (частая смена работы, пробелы, понижение в должности, неряшливость, общие фразы).
7.  **Interview Strategy**: Придумай 3 острых вопроса, чтобы проверить слабые места или подтвердить сильные.
