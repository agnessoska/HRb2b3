-- Удаляем политику, вызывающую рекурсию для HR
DROP POLICY IF EXISTS "candidates_can_view_chat_hr" ON hr_specialists;

-- Удаляем функцию get_my_candidate_id
DROP FUNCTION IF EXISTS get_my_candidate_id();

-- Создаем безопасную RPC функцию для получения чатов с данными собеседника
...
RETURNS TABLE (
  ...
  other_user_id uuid,
  other_user_category text -- Название категории для кандидата
)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  IF v_is_hr THEN
    ...
    SELECT 
      ...
      pc.name_ru as other_user_category -- Берем RU по дефолту, фронт может не использовать
    FROM chat_rooms cr
    JOIN candidates c ON c.id = cr.candidate_id
    LEFT JOIN professional_categories pc ON pc.id = c.category_id
    WHERE cr.hr_specialist_id IN (SELECT id FROM hr_specialists WHERE user_id = v_user_id)
  ...
  ELSE
    ...
    WHERE cr.candidate_id IN (SELECT id FROM candidates WHERE user_id = v_user_id)
    ORDER BY cr.last_message_at DESC NULLS LAST;
  END IF;
END;
$$;