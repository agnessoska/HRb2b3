-- Drop all existing variations of the function to resolve ambiguity
DROP FUNCTION IF EXISTS public.complete_user_profile(text, text);
DROP FUNCTION IF EXISTS public.complete_user_profile(text, text, text);

-- Recreate the function with the full signature
...
BEGIN
  ...
  IF p_invitation_token IS NOT NULL THEN
    ...
    IF FOUND THEN
        ...
        -- Fix: used_by column name
        UPDATE hr_invitation_tokens SET is_used = true, used_at = now(), used_by = v_user_id WHERE id = v_invite_record.id;
